...
- `對帳單.bat` — CP950/Big5 編碼，選單入口（1→`find_anomaly_members.ps1`，2→`filter_by_percent.ps1`，3→`filter_top5.ps1`（一鍵前5%含結餘），4→重選 CUB.MDB，5→`generate_statements.ps1`（Word 套印 PDF），6→結束）
- `對帳單.bat` 寫死 `DEFAULT_PWD=thifincub`；密碼不同時經 `get_password.ps1` 彈窗輸入
- `select_cub.ps1` / `get_password.ps1` 使用 WinForms → stdout 輸出由 bat 的 `for /f` 擷取
- 所有 `.ps1` **必須用 `pwsh` (PowerShell 7+)**。PS 5.1 會把 SQL 的 `<` 解析為重新導向而失敗
- `find_anomaly_members.ps1`、`loan_merge_check.ps1`、`balance_check.ps1` 支援 `-CubPath`；其餘腳本固定讀取同目錄 `CUB.MDB`。所有腳本皆支援 `-CubPassword`
...
`lib/AnomalyScore.psm1:$script:AnomalyWeights` 定義 17 項，僅保留與挪用相關者：
...
分級：Score≥10→High, 5-9→Mid, 1-4→Low。新增旗標須同時更新權重表和 `Get-AnomalyScore`。
...
| `Get-MemberValue` | `AnomalyScore.psm1` 有定義但**未匯出**；各腳本內建區域版本 |
| OUTDAT 過濾 | 在社社員用 `(OUTDAT IS NULL OR OUTDAT='')`，非 `TYPE='1'` |
...
- CSV 輸出欄位應含 `NegativeLoanBalance`、`LoanBeforeApproval`、`DormantActivation`、`RoundAmountTxn`、`NewAccountBurst`
- 分數分布包含 High / Mid / Low 三級，無退社社員混入
...
- `.gitignore` 排除 `CUB.MDB`、`CUB_異常社員_對帳單排序_*.csv`、`CUB_異常社員_篩選結果.csv`、`CUB_異常社員_Top*Pct_*.csv`
